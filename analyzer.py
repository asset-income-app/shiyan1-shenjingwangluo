...
def analyze_xor_network(W1, b1, W2, b2):
    ...
    test_points = np.array([[0, 0, 1, 1],
                            [0, 1, 0, 1]])
    z1 = np.dot(W1, test_points) + b1.reshape(-1, 1)
    a1 = relu(z1)
    
    for i in range(test_points.shape[1]):
        x1, x2 = test_points[0, i], test_points[1, i]
        print(f"输入 ({x1}, {x2}):")
        for j in range(a1.shape[0]):
            print(f"  隐藏层神经元 {j+1} 激活值: {a1[j, i]:.4f}")
    print()
    ...
    for i in range(test_points.shape[1]):
        x1, x2 = test_points[0, i], test_points[1, i]
        z1 = np.dot(W1, test_points[:, i:i+1]) + b1.reshape(-1, 1)
        a1 = relu(z1)
        z2 = np.dot(W2, a1) + b2
        a2 = sigmoid(z2)
...
def infer_rule(output_patterns):
    ...
    for p in output_patterns:
        if p['output'] > 0.5:
            x1, x2 = p['input']
            rule_parts.append(f"({x1}=={x2[0]} and {x2}=={x2[1]})")
